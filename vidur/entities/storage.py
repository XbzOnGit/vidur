...
class Trie:
    ...
    def delete(self, key: List[str], evict_storage):
        ...
        if self.delete_callback is not None:
            ...
            self.delete_callback(key, evict_storage, None, None)


def prefix_lookup(key: List[str], trie: Trie, lru_timestamp, evict_storage) -> Union[List[str], None]:
...
# evict_storage
def lru_evict_selection(lru_from_token_tuple_to_timestamp) -> List[str]:
...
def random_evict_selection(random_evict_tokens) -> List[str]:
    return random.choice(list(random_evict_tokens))

def FIFO_evict_selection(fifo_queue_tokens) -> List[str]:
...
class Storage(BaseEntity):
    ...
    def lookup(self, key:List[str]) -> Union[List[str], None]:
        ...
        item = self._lookup(key, self._lookup_storage, self._lru_timestamp, self._evict_storage)
        return item
    ...
    def insert(self, key:List[str]):
        ...
        while needed_size + self._used_space > self._capacity:
            evicted_one = self._evict_selection(self._evict_storage)
